...
- **Autosync worker** (`telegram/autosync.py`) — an asyncio background task started alongside bot polling. Every `TELEGRAM_AUTOSYNC_INTERVAL` seconds (default 600) it iterates all registered profiles with `autosync=True`, detects a new latest Coros activity (baseline stored under `coros:latest_activity:{email}`), uploads the last two days of activities to Garmin (duplicates are skipped by Garmin), and notifies the user in Telegram. Configured per user with `/autosync` (inline keyboard: on / on-quiet / off; quiet mode syncs without per-workout notifications). `/status` reports Coros/Garmin session state, autosync mode, and last sync time; `/start`+`/help` list all commands (also registered as the Telegram command menu via `set_my_commands`). `/stats` shows aggregated workout stats (this week / last week / this month) with a per-user digest toggle: when on, the autosync worker sends a daily summary every evening (after `TELEGRAM_DIGEST_HOUR` UTC, default 17) and a weekly one on Sundays. Uploaded activities are renamed to pretty titles ("🏃 Morning Run · 10.2 km", `build_activity_title` in telegram/utils.py). Owner alerts (`TELEGRAM_OWNER_ID`): autosync auto-disable events and 3+ consecutive per-user sync failures. Auto-disables (with a notification) on Coros auth failure or expired Garmin session. Note: on Render's free tier the service sleeps after 15 min without inbound HTTP traffic — an external keep-alive ping to the CherryPy `/` endpoint (e.g. UptimeRobot every 5 min) is required for autosync to run continuously.
...
- **Web (`webapp/`)** — an aiohttp server running inside the bot's event loop (started from `telegram/app.py`, replaces the old CherryPy stub). Always serves `GET /` (200 "OK") for Render port detection and UptimeRobot keep-alive. Behind the `TELEGRAM_WEBAPP_ENABLED` feature flag it also serves the **Telegram Mini App**: `webapp/static/` (vanilla HTML/JS dashboard + settings, Telegram theme vars) at `/webapp`, and a JSON API (`/api/me`, `/api/stats`, `/api/activities`, `/api/sync/today`, `/api/sync/latest`, `/api/settings`) authenticated via Telegram WebApp initData (`webapp/auth.py`, aiogram's `safe_parse_webapp_init_data` + allow-list + profile check). The bot's chat menu button opens the app when `TELEGRAM_WEBAPP_URL` is set. Note: the frontend deliberately does NOT live in the repo-level `static/` dir — that is a scratch area for users' downloaded activity files.
